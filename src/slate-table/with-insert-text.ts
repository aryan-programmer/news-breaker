...
export function withInsertText<T extends Editor>(editor: T, { withInsertText }: WithTableOptions): T {
	if (!withInsertText) {
		return editor;
	}
...
function shouldCollapse(editor: Editor): boolean {
	...
	return TableCursor.isInTable(editor, { at: selection }) && !hasCommon(editor, [startPoint.path, endPoint.path], "th", "td");
}
