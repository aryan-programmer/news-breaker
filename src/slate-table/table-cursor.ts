...
export const TableCursor = {
	...
	upward(editor: Editor, options: { mode?: SelectionMode } = {}): boolean {
		...
		if (options.mode === "all") Transforms.select(editor, path);
		else Transforms.select(editor, options.mode === "start" ? Editor.start(editor, path) : Editor.end(editor, path));
	...
	forward(editor: Editor, options: { mode?: SelectionMode } = {}): boolean {
		...
		if (options.mode === "all") Transforms.select(editor, nextPath);
		else Transforms.select(editor, options.mode === "start" ? Editor.start(editor, nextPath) : Editor.end(editor, nextPath));
	...
	backward(editor: Editor, options: { mode?: SelectionMode } = {}): boolean {
		...
		if (options.mode === "all") Transforms.select(editor, previousPath);
		else Transforms.select(editor, options.mode === "start" ? Editor.start(editor, previousPath) : Editor.end(editor, previousPath));
	...
	isOnEdge(editor: Editor, edge: Edge): boolean {
		...
		const point = edge === "start" || edge === "top" ? Range.start(selection) : Range.end(selection);
	...
	*selection(editor: Editor): Generator<NodeEntry[]> {
		...
		if (!editorOptions?.withSelection) {
			throw new Error("The `selection` command must be used with the `withSelection` option.");
		}
	...
	unselect(editor: Editor): void {
		const editorOptions = EDITOR_TO_WITH_TABLE_OPTIONS.get(editor);
		if (!editorOptions?.withSelection) {
			throw new Error("The `unselect` command must be used with the `withSelection` option.");
		}
	...
	isSelected<T extends Element>(editor: Editor, element: T): boolean {
		const editorOptions = EDITOR_TO_WITH_TABLE_OPTIONS.get(editor);
		if (!editorOptions?.withSelection) {
			throw new Error("The `isSelected` command must be used with the `withSelection` option.");
		}
