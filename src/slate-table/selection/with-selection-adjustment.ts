 ...
 */
export function withSelectionAdjustment<T extends Editor>(editor: T, { withSelectionAdjustment }: WithTableOptions): T {
	...
	editor.apply = (op: Operation): void => {
		...
		if (!Range.isRange(selection) || Range.isCollapsed(selection) || hasCommon(editor, [selection.anchor.path, selection.focus.path], "table")) {
			return apply(op);
		}
